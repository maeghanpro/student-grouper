...
import {
  Card, CardContent, CardActions, Button, Typography, IconButton, Tooltip, 
...
const ClassSectionTile = ({ classSection, patchClassSection, errors, deleteClass }) => {
  ...
  let groupsButton = (
    <Link to={`/classes/${classSection.id}/groups`}>
      <Button className="class-card-button" variant="contained" size="large">
        Groups
      </Button>
    </Link>
  );
