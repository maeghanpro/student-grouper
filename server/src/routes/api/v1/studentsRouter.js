...
studentsRouter.post("/", async (req, res) => {
  ...
  try {
    ...
    const students = await Student.query()
      ...
      .orderBy("firstName");
    const serializedStudents = students.map((student) => StudentSerializer.getSummary(student));
...
studentsRouter.patch("/", async (req, res) => {
  ...
  try {
    ...
    const students = await classSection.$relatedQuery("students").orderBy("firstName");
    const serializedStudents = students.map((student) => StudentSerializer.getSummary(student));
