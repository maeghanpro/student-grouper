...
arrangementsRouter.post("/", async (req, res) => {
  ...
  try {
    ...
    serializedArrangement.groups = await Promise.all(
      groups.map(async (group, index) => {
        const name = `Group ${index + 1}`;
        const persistedGroup = await Group.query().insertAndFetch({
          name,
    ...
    return res
      .status(201)
      .json({ featuredArrangement: serializedArrangement, classSection: serializedClassSection });
  } catch (error) {
...
arrangementsRouter.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    await Promise.all(relatedGroups.map((group) => group.$relatedQuery("assignments").delete()));
...
arrangementsRouter.patch("/:id", async (req, res) => {
  ...
  try {
    ...
    const arrangements = await Arrangement.query().where(
      "classSectionId",
      arrangement.classSectionId
    );
    const serializedArrangements = await Promise.all(
      arrangements.map((arrangement) => ArrangementSerializer.getDetails(arrangement))
    );
